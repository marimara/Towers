using UnityEditor;
using UnityEngine;

/// <summary>
/// Custom Inspector for <see cref="EventData"/>.
/// Renders the auto-generated ID as a selectable, read-only field so designers
/// can copy it for reference without being able to edit it accidentally.
/// All other fields are drawn by the default Inspector pipeline.
/// </summary>
[CustomEditor(typeof(EventData))]
public class EventDataEditor : Editor
{
    // Cached style — created once per Inspector instance.
    private GUIStyle _readOnlyStyle;

    public override void OnInspectorGUI()
    {
        serializedObject.Update();

        DrawReadOnlyId();

        EditorGUILayout.Space(4);

        // Draw every serialised field except _id (HideInInspector keeps it out
        // of the default iterator) and m_Script (redundant header clutter).
        DrawPropertiesExcluding(serializedObject, "m_Script");

        serializedObject.ApplyModifiedProperties();
    }

    // -------------------------------------------------------------------------
    // Private helpers
    // -------------------------------------------------------------------------

    private void DrawReadOnlyId()
    {
        _readOnlyStyle ??= new GUIStyle(EditorStyles.helpBox)
        {
            wordWrap     = false,
            stretchWidth = true,
            fontSize     = EditorStyles.label.fontSize,
            normal       = { textColor = new Color(0.6f, 0.6f, 0.6f) },
        };

        var eventData = (EventData)target;

        using (new EditorGUILayout.HorizontalScope())
        {
            EditorGUILayout.PrefixLabel(new GUIContent(
                "ID",
                "Auto-generated GUID. Read-only — assigned on asset creation."));

            EditorGUILayout.SelectableLabel(
                eventData.Id,
                _readOnlyStyle,
                GUILayout.Height(EditorGUIUtility.singleLineHeight));
        }
    }
}
